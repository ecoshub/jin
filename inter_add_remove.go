			...
			for i := offset ; i < len(json) ; i ++ {
				...
				if inQuote {
					...
					if curr == 93 || curr == 125 {
						level--
						if level == 0 {
							return replace(json, []byte(fmt.Sprintf(`,"%v":%v`, key, string(value))),i,i), nil
						}
						continue
					}
					continue
...
func AddValue(json []byte, value []byte, path ... string) ([]byte, error){
	...
	if len(path) == 0 {
		if json[offset] == 91 {
			...
			for i := offset ; i < len(json) ; i ++ {
				...
				if inQuote {
					...
					if curr == 93 || curr == 125 {
						level--
						if level == 0 {
							return replace(json, []byte(fmt.Sprintf(`,%v`, string(value))),i,i), nil
						}
						continue
					}
					continue
			...
			return json, errors.New("Error: Error: last path must be pointed at an array.")
		}
	}
	currentPath := path[0]
	braceType := json[offset]
...
func AddValueString(json []byte, value string, path ... string) ([]byte, error){
	if value[0] != 34 && value[len(value) - 1] != 34 {
		return Set(json, []byte(`"` + value + `"`), path...)
	}
	return Set(json, []byte(value), path...)
}
